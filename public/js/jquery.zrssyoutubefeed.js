(function(h){h.fn.youtubefeed=function(q,f,r){f=h.extend({limit:10,header:!0,titletag:"h4",date:!0,content:!0,snippet:!0,media:!0,showerror:!0,errormsg:"",key:null,ssl:!1,linktarget:"_blank"},f);return this.each(function(s,l){var p=h(l),d="";
f.ssl&&(d="s");
p.hasClass("youtubefeed")||p.addClass("youtubefeed");
if(null==q)return!1;
...
var ytubeid = c.link.substring(c.link.indexOf("?v=")+3,c.link.indexOf("&",c.link.indexOf("?v=")));
a+='<a style="float:left; width:85px;" href="'+c.link+'" target="_blank" title="View this feed at '+g.title+'"><img style="float:left;width:75px;height:75px;" src="http://img.youtube.com/vi/'+ytubeid+'/1.jpg" />';
a+='<p class="date"></p></a>';
b.date&&i&&(a+="<strong class='date'>"+i+"</strong>");
a+='<a href="'+c.link+'" target="_blank" title="View this feed at '+g.title+'"><span>'+c.title+'</span>';
a+='<p class="date"></p></a>';
...
if(b.media&&0<j.length){c=j[e].getElementsByTagName("enclosure");
if(0<c.length){for(var a=a+'<div class="rssMedia"><div>Media files</div><ul>',
k=0;k<c.length;k++)var m=c[k].getAttribute("url"),n=c[k].getAttribute("type"),o=c[k].getAttribute("length"),m='<li><a href="'+m+'" title="Download this media">'+m.split("/").pop()+"</a> ("+n+", ",n=Math.floor(Math.log(o)/Math.log(1024)),o=(o/Math.pow(1024,Math.floor(n))).toFixed(2)+" "+"bytes,kb,MB,GB,TB,PB".split(",")[n],a=a+(m+o+")</div></li>");a+="</ul></div>"}a+="</div></li>"}d="odd"==d?"even":"odd"}h(l).html(a+"");h("a",l).attr("target",b.linktarget)}h.isFunction(r)&&r.call(this,p)}else f.showerror&&

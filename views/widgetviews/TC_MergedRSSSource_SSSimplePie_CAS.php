...
if(!isset($TC_MergedRSSSource_SSSimplePie_CAS_widgetEntity_ID))
{
    
    throw new Exception("cannot load merged feed widget.");
    
}

if(isset($TC_MergedRSSSource_SSSimplePie_CAS_widget))
{
    $model = $TC_MergedRSSSource_SSSimplePie_CAS_widget;
}
else
{
    $modelBuilder = new TC_MergedRSSSource_Builder($TC_MergedRSSSource_SSSimplePie_CAS_widgetEntity_ID);
...
$feedStrategy = new SimplePieMergedStrategy($model);
...
<div class="widget">
    <h1><?php echo $model->get_title() ;?><div class="sourceExpandIcon opened"></div></h1>
              <ul id="widget-<?php echo $TC_MergedRSSSource_SSSimplePie_CAS_widgetEntity_ID ;?>" class="mediafeed">
            <?php 
//		    for($i = 0; $i < GLOBAL_FEED_ITEM_CAP && $i < count($feed) ; $i++)
//			{
//				$item = $feed[$i];
                    ...
                    <div class="feedContent">
                        ...
                        <a target="_blank" href="<?php echo $item->get_link(); ?>">read full article</a>
                         <?php
                            ...
                            {
                            ?>
                        <div style="float:right; padding-right:5px;"><a href="<?php echo $item->get_link() . '#comments' ;?>" target="_blank" title="Discuss in Communities @ State">comments:
                            <?php
			...
			<a class="share tooltip" title="Send by email"  href="mailto:?Subject=Shared From the Current - <?php echo strip_tags($purifier->purify($item->get_title())) ;?>&body=<?php echo strip_tags($item->get_link()); ?>"><div class="shareIcon mailShare"></div><span class="tttext classic">Send by email</span></a>	
                        <a class="share tooltip" title="Discuss in Communities @ State" href="<?php echo $item->get_link() . '#comments' ;?>" target="_blank"  ><div class="shareIcon discussShare"></div><span class="tttext classic">Discuss in Communities @ State</span></a>
		    </div>
                          </div>  
